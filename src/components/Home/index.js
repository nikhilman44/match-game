...
class Home extends Component {
  ...
  shuffleImage = () => imagesList.sort(() => Math.random() - 0.5)
  ...
  onClickImage = clickedId => {
    ...
    const initialImageList = imagesList
    const shuffleImage = initialImageList.sort(() => Math.random() - 0.5)

    const shuffledImage = shuffleImage

    if (id === clickedId) {
      this.setState(prevState => ({
        score: prevState.score + 1,
        randomImage: shuffledImage[randomNumber],
      }))
    } else {
      this.setState({isMatched: false})
      this.clearTimerInterval()
  ...
  render() {
    const {score, time, randomImage, activeTabId, isMatched} = this.state

    const {imageUrl} = randomImage

    const filteredImageList = this.getActiveTabImages()
    ...
    return (
      <div className="home">
        ...
        {!isMatched ? (
          ...
          <>
            ...
            <div className="card-container-3">
              <ul className="image-unordered-list">
        ...
        <div className="poori-container">
          <p className="poorig">By</p>
          <h1 className="poori">Poornima Ranguri</h1>
        </div>
      </div>
    )
  }
